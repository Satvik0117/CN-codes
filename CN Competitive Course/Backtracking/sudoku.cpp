...
bool findEmptyLocation(int grid[N][N], int &row, int &col){	//to recieve in solve sudoku fn we have passed by reference
	for(int i=0;i<N;i++){
		for(int j=0;j<N;j++){
			if(grid[i][j]==0){
			    row=i;
...
bool isSafeInGrid(int grid[N][N],int row,int col,int num){
	...
	for(int i=0;i<3;i++){
		for(int j=0;j<3;j++){
			if(grid[i+rowFactor][1+colFactor] == num)
				return false;
		}
	}
...
bool solveSudoku(int grid[N][N]){
	int row,col;
	if(!findEmptyLocation(grid,row,col)){	//passed using reference to recieve multiple values in this function 
		return true;
	}
	for(int i=1;i<=9;i++){
